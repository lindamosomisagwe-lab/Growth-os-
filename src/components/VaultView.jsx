  ...
  if (loading) {
    return (
      <div className="content-wrap" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '50vh' }}>
        <div style={{ color: 'var(--text-muted)', fontSize: '14px' }}>Opening Time Vault...</div>
      </div>
    );
  }
  ...
  return (
    <div className="content-wrap" style={{ padding: '32px 24px 80px', maxWidth: '640px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '32px' }}>
        <div>
          <h1 className="page-title" style={{ fontSize: '32px', fontWeight: 700, fontFamily: "'Playfair Display', Georgia, serif", color: '#1B1D1D', margin: 0 }}>Time Vault</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: '14px', marginTop: '4px' }}>Seal safe memories for your future self</p>
        </div>
        <motion.button 
          whileHover={{ scale: 1.02, backgroundColor: 'rgba(96,122,102,0.1)' }}
          whileTap={{ scale: 0.98 }}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {letters.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '64px 0', border: '1px dashed rgba(27,31,29,0.15)', borderRadius: '16px', background: 'rgba(27,31,29,0.01)' }}>
            ...
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setShowModal(true)} style={{ background: 'rgba(96,122,102,0.15)', color: '#607A66', border: 'none', padding: '10px 20px', borderRadius: '8px', cursor: 'pointer', fontWeight: 600 }}>
          ...
          letters.map((letter, index) => {
            ...
            return (
              <motion.div 
                ...
                whileHover={isReady || isOpened ? { scale: 1.01, backgroundColor: 'rgba(27,31,29,0.02)' } : {}}
                whileTap={isReady || isOpened ? { scale: 0.99 } : {}}
                style={{
                  background: '#FFFFFF',
                  border: `1px solid ${isReady ? 'var(--amber)' : 'rgba(27,31,29,0.08)'}`,
                  borderLeft: `4px solid ${isOpened ? '#607A66' : isReady ? '#E6DCC3' : '#C4596A'}`, 
                  borderRadius: '12px',
                  padding: '20px',
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                    <div style={{ fontSize: '11px', color: isOpened ? '#607A66' : isReady ? 'var(--amber)' : '#C4596A', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700 }}>
      ...
      <AnimatePresence>
        {showModal && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', padding: '24px' }}>
            ...
            <motion.div initial={{ y: '100%', opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: '100%', opacity: 0 }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} style={{ background: '#FFFFFF', border: '1px solid rgba(27,31,29,0.12)', borderBottom: 'none', padding: '32px 24px 48px', borderTopLeftRadius: 24, borderTopRightRadius: 24, width: '100%', maxWidth: 640, position: 'relative', zIndex: 101 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <h3 style={{ margin: 0, fontSize: '20px', fontWeight: 700, color: '#1B1D1D', fontFamily: "'Playfair Display', Georgia, serif" }}>Seal a Letter in the Vault</h3>
              ...
              <input placeholder="Subject (e.g. Note to 30-year-old me)" value={subject} onChange={e => setSubject(e.target.value)} style={{ width: '100%', background: 'rgba(27,31,29,0.01)', border: '1px solid rgba(27,31,29,0.12)', padding: 14, borderRadius: 10, color: '#1B1D1D', marginBottom: 16, outline: 'none', fontSize: '14px', boxSizing: 'border-box' }} />
              
              <textarea placeholder="Write what's on your mind... (This letter will be sealed and completely hidden until the unlock date)" value={body} onChange={e => setBody(e.target.value)} style={{ width: '100%', background: 'rgba(27,31,29,0.01)', border: '1px solid rgba(27,31,29,0.12)', padding: 14, borderRadius: 10, color: '#1B1D1D', marginBottom: 16, outline: 'none', fontSize: '14px', minHeight: '160px', resize: 'vertical', boxSizing: 'border-box' }} />
              ...
              <div style={{ marginBottom: '24px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px', marginBottom: '12px', overflowX: 'auto', paddingBottom: '4px' }}>
                  {[[3, '3 Months'], [6, '6 Months'], [12, '1 Year'], [24, '2 Years']].map(([months, label]) => (
                    <button key={months} onClick={() => setPresetDate(months)} style={{ flexShrink: 0, background: 'rgba(27,31,29,0.02)', border: '1px solid rgba(27,31,29,0.08)', padding: '6px 14px', borderRadius: '16px', color: '#1B1D1D', fontSize: '13px', cursor: 'pointer', outline: 'none' }}>{label}</button>
                  ))}
                </div>
                <input type="date" value={revealDateStr} onChange={e => setRevealDateStr(e.target.value)} style={{ width: '100%', background: 'rgba(27,31,29,0.01)', border: '1px solid rgba(27,31,29,0.12)', padding: 14, borderRadius: 10, color: '#1B1D1D', outline: 'none', boxSizing: 'border-box' }} />
              </div>
              ...
              <motion.button 
                ...
                style={{ 
                  ...
                  fontSize: '16px', fontWeight: 700, cursor: (!subject || !body || !revealDateStr || isSaving) ? 'not-allowed' : 'pointer'
                }}
              >
            ...
            <motion.div initial={{ y: '100%', opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: '100%', opacity: 0 }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} style={{ background: '#FFFFFF', border: '1px solid rgba(27,31,29,0.12)', borderBottom: 'none', padding: '32px 24px 48px', borderTopLeftRadius: 24, borderTopRightRadius: 24, width: '100%', maxWidth: 640, maxHeight: '80vh', overflowY: 'auto', position: 'relative', zIndex: 101 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
                <div>
                  ...
                  <h3 style={{ margin: 0, fontSize: '22px', fontWeight: 700, color: '#1B1D1D', fontFamily: "'Playfair Display', Georgia, serif" }}>{selectedLetter.subject}</h3>
                </div>
              ...
              <div style={{ fontSize: '15px', color: '#5C615C', lineHeight: 1.6, whiteSpace: 'pre-wrap', fontStyle: 'italic', background: 'rgba(27,31,29,0.01)', padding: '20px', borderRadius: '12px', border: '1px solid rgba(27,31,29,0.06)' }}>
                "{selectedLetter.body}"
              </div>
            </motion.div>
          </div>
